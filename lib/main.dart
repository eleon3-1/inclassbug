...
class _ControlDeckScreenState extends State<ControlDeckScreen> {
  ...
  // 🐛 BUG #1 FIX:
  // ispressed does not to be here anymore and is moved to the tactilite widget
        ...
        child: Column(
          children: [
            ...
            Slider(
              ...
              // 🐛 BUG #2 FIX:
              //setState() tells flutter to rebuild the UI so that it can change
              onChanged: (newVal) {
                setState(() {
                  powerLevel = newVal;
                });
...
class _TactileButtonState extends State<TactileButton> {
  // 🐛 BUG #1 FIX:
  // Each button now owns its own isPressed variable.
  // Pressing one button will not affect the other buttons.
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      // 🐛 BUG #4 FIX:
      // onTapDown only changes the visual pressed state.
      // It does NOT fire the action.
      onTapDown: (_) {
        setState(() {
          isPressed = true;
        });
      ...
      // 🐛 BUG #4 FIX:
      // the action will go when the user doesnt press the button
      onTapUp: (_) {
        setState(() {
          isPressed = false;
        });
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          // 🐛 BUG #3 FIX:
          //Pressed state uses smaller offset and the unpressed state needs to
          // user bigger offset numbers
...
class ThemeChallengePanel extends StatefulWidget {
  ...
  @override
  State<ThemeChallengePanel> createState() =>
      _ThemeChallengePanelState();
}
    ...
    return Column(
      children: [
        ...
        ElevatedButton(
          ...
          child: const Text("Laser Blast"),
        ),

        ElevatedButton(
          onPressed: () {
            setState(() {
              forcefieldActive = !forcefieldActive;
            });
          },
          child: const Text("Forcefield"),
        ),
      ],
    );
  }
